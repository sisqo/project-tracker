...
export function NewProjectForm({ requesters }: { requesters: Requester[] }) {
  ...
  return (
    <form action={formAction} className="flex max-w-xl flex-col gap-4 rounded-lg border border-slate-200 bg-white p-6">
      ...
      <div className="flex gap-4">
        <div className="flex flex-col gap-1">
          ...
          <input name="color" type="color" defaultValue="#6366f1" className="h-9 w-16 rounded border border-slate-300" />
        </div>
        <div className="flex flex-1 flex-col gap-1">
          <label className="text-sm font-medium text-slate-700">Priorità</label>
      ...
      <div className="flex flex-col gap-1">
        <label className="text-sm font-medium text-slate-700">Richiedente</label>
        <select name="requesterId" required className="rounded border border-slate-300 px-3 py-2 text-sm">
          {requesters.map((r) => (
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea name="description" rows={4} className="rounded border border-slate-300 px-3 py-2 text-sm" />
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-slate-600">Data richiesta</label>
          <input name="requestDate" type="date" className="rounded border border-slate-300 px-2 py-1 text-sm" />
        </div>
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-slate-600">Data avvio</label>
          <input name="startDate" type="date" className="rounded border border-slate-300 px-2 py-1 text-sm" />
        </div>
        <div className="flex flex-col gap-1">
      ...
      <button
        type="submit"
        disabled={pending}
        className="mt-2 self-start rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500 disabled:opacity-60"
      >
